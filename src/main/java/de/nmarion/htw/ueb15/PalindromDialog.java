...
public class PalindromDialog {
    ...
    public void scanFile(final Palindrom palindrom, final String filename) {
        ...
        try (Stream<String> stream = Files.lines(Paths.get(file.getAbsolutePath()))) {
            stream.forEach(line -> {
                System.out
                        .println(line + (palindrom.istPalindrom(line) ? " ist ein Palindrom" : " ist kein Palindrom"));
            });
    ...
    public static void main(String... args) {
        ...
        final Palindrom palindrom = args[0].equalsIgnoreCase("iterativ") ? new PalindromIterativ()
