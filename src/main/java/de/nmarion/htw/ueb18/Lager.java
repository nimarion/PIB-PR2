...
public class Lager {
  ...
  public List<Artikel> getArticles(
      final Predicate<Artikel> searchPredicate, final BiPredicate<Artikel, Artikel> sortPredicate) {
    return Arrays.asList(getSorted(sortPredicate)).stream()
        .filter(artikel -> searchPredicate.test(artikel))
